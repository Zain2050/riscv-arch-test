...
// define a bunch of XLEN dependent constants
    ...
    #define LREG lq
#endif
#define XLEN_WIDTH 5+(XLEN>>6) //  width of the XLEN field, =5,6,7 for RV32,64,128
        
// Now define a bunch of FLEN dependent constants

#define FREGWIDTH (FLEN>>3)
...
#if SIGALIGN==8
  #define CANARY   .dword 0x6F5CA309E7D4B281
#else
...
/**** to a spcl rtn_fm_mmode: handler if a1=0. That code translates MEPC from   ****/
...
/**** **NOTE**: this destroys T2 and clears a1 (param register)                 ****/
...
#ifndef GOTO_M_OP
    #define GOTO_M_OP   ecall  // default; this must be called with a1=0
#endif
...
//**************** handle mode field
  ...
  #endif
  #ifdef rvtest_strap_routine                             // ensure you don't go to S in an M-U system
  .elseif (\LMODE\() == Hmode )    // get trapsig_ptr & init val up 1 save areas (M->S)
...
spcl_\__MODE__\()2mmode_test:
        csrr    T5, CSR_XCAUSE
        LI(T4,(1<<(XLEN-1))+(1<<12))            // make a mask of int bit and cause(11:0).
...
.else
   ...
   .else
    ...
    .endif
.endif
        addi    sp, sp, sv_area_off             // this offsets Sp to avoid overflowing the offset parameter
...
//  convert mtrap_sigptr to curr_mode trap_sigptr
        ...
        addi    sp, sp, -sv_area_off            // undo the sp offset
...
sv_\__MODE__\()vect:                            // **FIXME?: breaks if tramp crosses pg && MMU enabled
        ...
        csrr    T4, CSR_XSTATUS                 // deposit xstatus(17:0) into [30:13)
        slli    T2, T2, XLEN-17 
...
.endif
        ...
        slli    T4, T4, 14-6
        or      T3, T3, T4
        TRAP_SIGUPD T4,  0                     // save 1st sig value, (vec-offset, entrysz, trapmode)

//----------------------------------------------------------------
sv_\__MODE__\()cause:
        mv      T3, T5                          // move mcause (T5) into T3 so all trap sig stores use T3 
        TRAP_SIGUPD T4, 1                       // save 2nd sig value, (mcause)
//----------------------------------------------------------------
...
.ifc \__MODE__ , M
 ...
 // select MPP based on MPRV; if MPRV=1, substitute saved mstatus (with MPP bits)
        slli    T4, T6, XLEN-MPRV_LSB-1         /* put MPRV [17] into sign bit & test   */
        bgez    T4, 1f
        LREG    T6, sved_mpp_off(sp)            /* use saved MPP, since MPP overwritten if MPRV=1 */

1: 
 // extract & test selected MPP=3, force reloc if coming from Mmode
        srli    T4, T6,  MPP_LSB
        andi    T4, T4,  MMODE_SIG
        addi    T4, T4, -MMODE_SIG
        bnez    T4, vmem_adj_\__MODE__\()epc
 ...
 // extract and test satp.MODE from trapping mode; if !=bare, VA, skip reloc
        ...
        bgez    T6, 1f                 // keep  SATP      if no hypervisor
          csrr  T4, CSR_HGATP          // subsitute HGATP if    hypervisor
1:      srli    T4, T4, MODE_LSB
        bnez    T4, sv_\__MODE__\()epc // skip reloc if not bare mode

 // extract and test mstatus.MPV; if 0, single translation & bare mode, force reloc
        csrr    T6, CSR_MSTATUS
        srli    T4, T6, XLEN-MPV_LSB-1
        beqz    T4, vmem_adj_\__MODE__\()epc

 // 2 lvl translation;  extract and test vsatp.MODE!=bare; if so, VA, skip reloc
        csrr    T4, CSR_VSSATP
        srli    T4, T4, MODE_LSB
        bnez    T4, skp_adj_\__MODE__\()epc     // mode is bare, fall through to force reloc
...
.ifc \__MODE__ ,  V
 ...
 // extract and test higher level satp.mode!=bare
        LREG    T4, sved_hgapt_off(sp)     /*saved when HS chgs its SATP */
        srli    T4, T4, MODE_LSB
        bnez    T4, skp_adj_\__MODE__\()epc
 // mode is bare, fall through to force reloc
  .endif
...
vmem_adj_\__MODE__\()epc:
        csrr    T3, CSR_XEPC            /* sp is still pointing to trapping mode sv_area  */

        LREG    T2, vmem_bgn_off(sp)            // see if epc is in the vmem area
        LREG    T6, vmem_seg_siz(sp)
...
code_adj_\__MODE__\()epc:
        LREG    T2, code_bgn_off(sp)            // see if epc is in the code area
        LREG    T6, code_seg_siz(sp)
...
data_adj_\__MODE__\()epc:
        LREG    T2, data_bgn_off(sp)            // see if epc is in the data area
        LREG    T6, data_seg_siz(sp)
...
adj_\__MODE__\()epc:
        sub     T4, T3, T2                      // Offset adjustment

sv_\__MODE__\()epc:
       TRAP_SIGUPD T4, 2                        // save 3rd sig value, (rel mepc) into trap sig area
        ...
        beq     T3, T6, 1f
        LI(     T6, CAUSE_GUEST_FETCH_ACCESS)
        bne     T3, T6, adj_\__MODE__\()epc_rtn // CAUSE_FETCH_ACCESS = 0x14         
...
vmem_adj_\__MODE__\()tval:         /* sp still points to sv area of trapping mode */
        LREG    T2, vmem_bgn_off(sp)              // fetch sig_begin addr
        LREG    T6, vmem_seg_siz(sp)
        add     T6, T6, T2                        // construct vmem seg end
...
sig_adj_\__MODE__\()tval:
        LREG    T2, sig_bgn_off(sp)               // fetch sig_begin addr
        LREG    T6, sig_seg_siz(sp)
...
code_adj_\__MODE__\()tval:
        LREG    T2, code_bgn_off(sp)              // fetch code_begin addr
        LREG    T6, code_seg_siz(sp)
        add     T6, T6, T2                        // construct code seg end
...
sv_\__MODE__\()tval:
        TRAP_SIGUPD T4, 3               // save 4th sig value, (rel tval)
...
skp_\__MODE__\()tval:
  ...
  .ifnc \__MODE__ , S
    .ifnc \__MODE__ , V                 // must be either M with H enabled or H
        ...
        TRAP_SIGUPD T4, 4               // store 5th sig value, only if mmode handler and VS mode exists
        csrr    T3, CSR_MTINST
        TRAP_SIGUPD T4, 5               // store 6th sig value, only if mmode handler and VS mode exists
    .endif
  .endif
...
spcl_\__MODE__\()handler:               // case table branch to special handler code, depending on mcause
        ...
        addi    T3, T3, 14*4            // shortcut to avoid LA clrint_xtbl - this is might be 4 too large
...
clrint_tbl:                              //this code should only touch T2..T6
 ...
 //****************************************************************
#else
  #ifdef rvtest_htrap_routine   // M/H/S/U only
        .dword  0                        // int cause  0 is rsvd, error
        .dword  \__MODE__\()clr_Ssw_int  // int cause  1  Smode SW int
 ...
 //****************************************************************
  #else
    #ifdef rvtest_strap_routine   // M/S/U only
        .dword  0                        // int cause  0 is rsvd, error
        .dword  \__MODE__\()clr_Ssw_int  // int cause  1  Smode SW int
        .dword  1                        // int cause  2  no Vmode
        .dword  \__MODE__\()clr_Msw_int  // int cause  3  Mmode SW int
...
from_hs_s:	
        ...
        j       1f
from_u_s:	
	LREG    T6, code_bgn_off+1*sv_area_sz(sp)/* get VU/VS   mode code begin */
...
/**** actual start of test is boot code at rvtest_entry_pt:, located at the end to keep test size constant ****/
...
/*                ----------------> test inserted here <----------------              */
/**************************************************************************************/
/**** The RVTEST_CODE_END macro defines end of test code. It svs regs, transitions ****/
/**** to Mmode & instantiates epilog code using the RVTEST_TRAP_EPILOG() macro.    ****/
/**** Test code must either fall thru to this or jmp to it via lbl rvtest_code_end ****/
/**** An RVMODEL_HALT macro completes the test. The trap handlers for each mode    ****/
/**** mode are instantiated immediately following with RVTEST_TRAP_HANDLER() macro ****/
/**** As well as test abort code, boot code, and selfcheck code                    ****/
/**************************************************************************************/
...
//restore xTVEC, trampoline, regs for each mode in opposite order that they were saved
        ...
        #endif
          RVTEST_TRAP_EPILOG S    // actual v-mode prolog/epilog/handler code
        #ifdef rvtest_htrap_routine
          RVTEST_TRAP_EPILOG H        // actual s-mode prolog/epilog/handler code
        #endif
    #endif
...
exit_cleanup:
    RVMODEL_CLEAN_SIG             // make sure the signature is in memory, not in cache, so it is easy to extract
//  RVMODEL_HALT /****FIXME: uncomment when all tests have explicit RVMODEL_HALT removed ****//
   j halt_test                     // comment this out when RVMODEL_HALT is uncommented
...
rvtest_entry_pt: 
    ...
    j rvtest_init                 // now go back to test prolog & fall thru to actual test

/*********************************************************************************************************/
/************* test failure; report back for self-checking mode                           ****************/
/************* This needs to have a print routine defined, and a std fail report defined  ****************/
/*********************************************************************************************************/

        /****FIXME: this is example pseudo-code, not real code that will be a default.   ****/
        /**** It can be customised with RVMODEL_WR_STR definition or completely replaced ****/
        
#ifdef RVMODEL_SELFCHECK             // T5(cause)/sp(ptr)/T2(rtnaddr)/T6(vector) are live, T1/T3/T4, are dead
trap_test_failure:
//  save registers
    csrr   T3, CSR_XEPC              // read_epc in current mode
//  jal to hex2ascii(epc_stringptr)  // cvt binary in T3 to ascii hex starting at addr epc_stringptr
    csrr   T3, mstatus               // use it to print whether this a VA or a PA (if curmode=M, look at MPP)
                                     // print which mode this was in
                                     // if VMM mode, compute the PA
//  jal to hex2ascii(mstat_stringptr)// cvt binary in T3 to ascii hex starting at addr mstat_stringptr
    addi T1, T1, -4                  //look back 4 bytes to find the compare op; may need to look further if V or F)
//  figure out if this is a 2B or 4B opcode and fetch it into T3
//  jal to hex2ascii(xreg, op_stringptr) // print opcode
//  extract rs1 from opcode, then fetched saved xreg into T3
//  jal to hex2ascii(rs1_stringptr)  // cvt binary in T3 to ascii hex starting at addr rs1_stringptr
//  extract rs2 from opcode, then fetched saved xreg into T3
//  jal to hex2ascii(rs1_stringptr)  // cvt binary in T3 to ascii hex starting at addr rs1_stringptr
//  RVMODEL_WR_STR fail_strptr)      // print string to console
//  restore registers
  #ifdef FAIL_ENDS_TEST
    j     exit cleanup
  #else
    jr    T2
//**** FIXME:  all SIGUPD must replace nops following possible traps with lpad(0)!!!!****
  #endif
fail_str: "failure@EPC XXXX_XXXX_XXXX_XXXX, Xstatus=XXXX_XXXX_XXXX_XXXX\n"
          " opcode=    XXXX_XXXX, rslt=XXXX_XXXX_XXXX_XXXX, expct=XXXX_XXXX_XXXX_XXXX\n"
#endif
...
mtrap_sigptr:
  ...
  #else                                  /**** FIXME: can this be deleted or ****/
    .fill 64*(XLEN/32),4,0xdeadbeef      /**** does mess up existing tests?  ****/
  #endif       /**** tests should explicitly allocate exactly what they need ****/
.endm

// if more signature area is required, it should be inserted
// between RVTEST_TSIG_BEGIN and RVTEST_SIG_END
